...
def dianyingxinxi_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        msg['data']['pageSize']  =dianyingxinxi.page(dianyingxinxi, dianyingxinxi, req_dict, request)
...
def dianyingxinxi_autoSort2(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        leixing = set()
        try:
            cursor.execute("select goodtype from orders where userid = %d"%(request.session.get("params").get("id"))+" and tablename = 'dianyingxinxi' order by addtime desc")
        ...
        L = []
        cursor.execute("select * from dianyingxinxi where dianyingleixing in ('%s"%("','").join(leixing)+"') union all select * from dianyingxinxi where dianyingleixing not in('%s"%("','").join(leixing)+"')")
...
def dianyingxinxi_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        ...
        sql = "SELECT {0}, sum({1}) AS total FROM dianyingxinxi {2} GROUP BY {0}".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def dianyingxinxi_alipay(request):
    if request.method in ["POST", "GET"]:
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            return_url='http://localhost:8080/django0ozlt/dianyingxinxi/notify',
            #notify_url=''
        )
...
def dianyingxinxi_notify(request):
    if request.method in ["POST", "GET"]:
        ...
        return redirect('http://localhost:8080/django0ozlt/admin/dist/index.html#/dianyingxinxi')
